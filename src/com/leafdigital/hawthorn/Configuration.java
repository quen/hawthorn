...
public class Configuration
{
	...
	private static String getText(Element container) throws StartupException
	{
		...
		for(int i=0;i<children.getLength();i++)
		{
			...
			{
				throw new StartupException(ErrorCode.STARTUP_CONFIGFORMAT,
					"Element "+container.getTagName()+" must not include XML tags.");
			}
	...
	public Configuration(File configFile) throws StartupException
	{
		...
		try
		{
			...
			for(int i=0;i<children.getLength();i++)
			{
				...
				{
					LinkedList<ServerInfo> otherServersList=new LinkedList<ServerInfo>();
					...
					otherServers=otherServersList.toArray(
						new ServerInfo[otherServersList.size()]);
				}
				...
				else if(child.getTagName().equals("testkey"))
				{
					...
					if(channel==null || (!channel.matches(Hawthorn.REGEXP_USERCHANNEL) &&
						!channel.equals(Logger.SYSTEMLOG)))
					{
						throw new StartupException(ErrorCode.STARTUP_CONFIGFORMAT,
					...
					if(displayName==null || !displayName.matches(Hawthorn.REGEXP_DISPLAYNAME))
					{
						throw new StartupException(ErrorCode.STARTUP_CONFIGFORMAT,
			...
			{
				logger.log(Logger.SYSTEMLOG,Logger.Level.NORMAL, "Remote server: "+otherServers[i]);
			}
		}
	...
	public class TestKey
	{
		...
		void show(Hawthorn app) throws StartupException
		{
			try
			{
				...
				System.out.println();
				String key=app.getValidKey(channel,user,displayName,testKeyTime+"");
				System.out.println("         Key: "+ key);
				System.out.println();
				System.out.println("channel="+channel+"&user="+user+"&displayname="+
					URLEncoder.encode(displayName,"UTF-8")+"&keytime="+testKeyTime+
					"&key="+key);
				System.out.println();
			}
			catch(Exception e)
